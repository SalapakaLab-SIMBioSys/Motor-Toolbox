...
class motorClass:
    ...
    def __init__(self, l0, Kel, Fs, ATP, Pattach, K_Pd, Pback, motor_type):
        ...
        self.K_Pd = K_Pd # This is the constant multiplier for Pdetach = K_Pd * Pstep
        self.Pback = Pback
    ...
    def Pstep_2sided(self, Fm):
        if abs(Fm) > self.Fs: # Force exerted due to stretching the linkage > Stalling force Fs 
            P = 0 # Probability of stepping is 0 for force beyond stall force
            epsilon = 0
            return [P, epsilon]
        
        # if Fm < 0 and Fm > -1*self.Fs: # Force exerted due to linkage stretching < 0 --> it is taken equal to 0 i.e. "not stretched" stage
        #     Fm = 0
        
        koff = self.k0off * np.exp( (abs(Fm)*self.dl)/(self.Kb*self.T) ) # s^(-1) ; As per formula of Koff, from ref.paper[7]
    ...
    def Pdetach_2sided(self, Fm):
        # k = 0.04 #Original rate from paper
        # k = 0.01
        if abs(Fm) >= self.Fs:
            P = self.Pback
        else:
            P = self.K_Pd * self.Pstep_2sided(Fm)[0]
            P = max(P,self.Pback)
        return P
    
    def Pdetach_1sided(self, Fm):
        # k = 0.04 #Original rate from paper
        # k = 0.01
        if abs(Fm) >= self.Fs:
            P = self.Pback
        else:
            P = self.K_Pd * self.Pstep_1sided(Fm)[0]
            P = max(P,self.Pback)
        return P
...
#==============================================================================
def createMotor(l0, Kel, Fs, ATP, Pattach, K_Pd, Pback, motor_type):
   k = motorClass(l0, Kel, Fs, ATP, Pattach, K_Pd, Pback, motor_type) 
   return k
...
#==============================================================================    
# Function which uses the above code2state function to determine the 
# cargo location number, exact state, stall strength on each motor in that
# configuration, and cargo position.
# Inputs are code = value from 1 to N (for total N relative configurations)
# i.e. code = no. of some relative configuration
# and n = bound on extent
# Output = {[Location number on track], [state],[stall strength],}
#==============================================================================
def code2fullState(motor,code,n, Fload = 0.0002, tol = 1e-6):
    
    locNumber = np.arange(1,n+1,1)
    
    state, Nm = code2state(code,n)
    pos = state2pos(state)
    
    forceArray = np.zeros(n)
    forceLoc = np.where(state > 0)
    force = np.array(get_force_array(state,Fload,tol))
    forceArray[forceLoc] = force/motor.Fs  #How strong the force is wrt stall force
    
    cargoPos = equilibrium_cargo_modified(motor, pos, Fload)
    cargoArr = np.zeros(n)
    cargoLoc = int(cargoPos/motor.ds)
    if cargoLoc > 0 & cargoLoc <= n:
        cargoArr[cargoLoc] = 1
       
    fullState = {}
    fullState['locNumber'] = locNumber
    fullState['Configuration'] = state
    fullState['forceArray'] = forceArray
    fullState['cargoArr'] = cargoArr
    
    return np.array([locNumber,state,forceArray,cargoArr]), fullState
...
def eqm_cargo_modified(motor, p, Fload, tol = 1e-6):
    p = p*motor.ds
    p = np.sort(p)
    n = len(p)
    
    if Fload == 0 and (p[0] + motor.l0 >= p[-1]- motor.l0):
        x = 0.5*(p[0]+p[-1])
    
        return x
    
    if Fload == 0 and (p[0]*motor.ds + motor.l0 < p[-1]*motor.ds - motor.l0):
...
def equilibrium_cargo(motor, p, Fload, tol = 1e-6):
    ...
    if Fload == 0: 
    #Zero load force on cargo, 'eqm_cargo' only used here
        eq = dummy_eqm_cargo(motor, p, Fload, tol)
        return eq
...
#============================================================================== 
def equilibrium_cargo_modified(motor, p, Fload, tol = 1e-6):
    
    if Fload == 0: 
        #Zero load force on cargo, 'eqm_cargo_modified' only used here
        eq = eqm_cargo_modified(motor, p, Fload, tol)
        return eq
...
#==============================================================================
#Bunch of functions to get force array for a state

def get_unique_pos(pos):
    unique_pos = []
    for ii in pos:
        if ii not in unique_pos:
            unique_pos.append(ii)
    return unique_pos

def get_pos_freq(pos):
    pos_freq = [len(list(group)) for key, group in groupby(pos)]
    return pos_freq

def get_force_array(motor,state,Fload,tol):
    
    pos = state2pos(state) 
        # positions of motors corresponding to state obtained
    unique_pos = get_unique_pos(pos)
    # pos_freq = get_pos_freq(pos)
    
    if pos.size == 0:
            x_c = 0        
    else:
        x_c = equilibrium_cargo_modified(motor, pos, Fload, tol) 
            #Calculate the equilibrium position of cargo       
        
        Force = []
    for ii in unique_pos:
        Force.append(motor.springForce((ii)*motor.ds - x_c))
...
'''
#============================================================================== 
def getTransitionMatrix_2sided(motor, Fload, m, tol):
    Psingle = []
    selfState = []
    ...
    #==================== Compute Step Transition rates =======================
        for ii in range(0,n-1):
            if state[ii] > 0:
                ...
                new_x_c = equilibrium_cargo_modified(motor, newPos, Fload, tol) 
                #new equilibrium position obtained
                
                if newCode != code:
                    A[newCode-1,code-1] += state[ii] * (motor.Pstep_2sided(Fm)[0]) 
                    #[0] index in Pstep gives the probability and [1] index gives the epsilon
                else:
                    Psingle.append(motor.Pstep_2sided(Fm)[0])
                    selfState.append(code)
                    
                #Not used immediately in pdf calculation (?)
                D[newCode-1,code-1] += (new_x_c - x_c) * state[ii] * (motor.Pstep_2sided(Fm)[0])
                tempVar = np.array([code,newCode,1])
                if new_x_c != x_c:
                    stepStat.append(tempVar)
                    probCargoStep.append([(new_x_c - x_c), (motor.Pstep_2sided(Fm)[0]) * state[ii]])
    ...
    #==================== Compute Detachmemt Transition rates =================
        for ii in range(0,n):
            if state[ii] > 0:
                ...
                if newCode != code:
                    A[newCode-1,code-1] += state[ii] * motor.Pdetach_2sided(Fm)
                
                #Not used immediately in pdf calculation (?)
                D[newCode-1,code-1] += (new_x_c - x_c) * state[ii] * motor.Pdetach_2sided(Fm)
                tempVar = np.array([code,newCode,-1])
                if new_x_c != x_c:
                    stepStat.append(tempVar)
                    probCargoStep.append([(new_x_c - x_c),  state[ii] * motor.Pdetach_2sided(Fm)])  
    ...
    #==================== Compute attachment Transition rates =====================
        ...
        if sum(state) < m and sum(state) > 0:
            ...
            for jj in range(minReattachLoc-1, maxReattachLoc):
                ...
                shifted_x_c = equilibrium_cargo_modified(motor, shiftedPos, Fload, tol)
...
#============================================================================
def getTransitionMatrix(motor, Fload, m, tol, twoSided = False):
    Psingle = []
    selfState = []
    ...
    #==================== Compute Step Transition rates =======================
        for ii in range(0,n-1):
            if state[ii] > 0:
                ...
                #new equilibrium position obtained
                if twoSided:
                    Pstep = motor.Pstep_2sided(Fm)[0]  
                else:
                    Pstep = motor.Pstep_1sided(Fm)[0]
                    
                
                if newCode != code:
                    A[newCode-1,code-1] += state[ii] * Pstep 
                    #[0] index in Pstep gives the probability and [1] index gives the epsilon
                else:
                    Psingle.append(Pstep)
                    selfState.append(code)
                    
                #Not used immediately in pdf calculation (?)
                D[newCode-1,code-1] += (new_x_c - x_c) * state[ii] * Pstep
                tempVar = np.array([code,newCode,1])
                if new_x_c != x_c:
                    stepStat.append(tempVar)
                    probCargoStep.append([(new_x_c - x_c), Pstep * state[ii]])
    ...
    #==================== Compute Detachmemt Transition rates =================
        for ii in range(0,n):
            if state[ii] > 0:
                ...
                if twoSided:
                    Pdetach = motor.Pdetach_2sided(Fm)   
                else:
                    Pdetach = motor.Pdetach_1sided(Fm)
                    
                if newCode != code:
                    A[newCode-1,code-1] += state[ii] * Pdetach
                
                #Not used immediately in pdf calculation (?)
                D[newCode-1,code-1] += (new_x_c - x_c) * state[ii] * Pdetach
                tempVar = np.array([code,newCode,-1])
                if new_x_c != x_c:
                    stepStat.append(tempVar)
                    probCargoStep.append([(new_x_c - x_c),  state[ii] * Pdetach])
...
def getTransitionMatrix_relSpace(motor, Fload, m, bigRelSpace, tol, twoSided = False):
    # Psingle = []
    # selfState = []
    ...
    for code in range(1,N+1): 
        ...
        for ii in range(0,n-1):
            ...
            if state[ii] > 0:
                ...
                # newCode = state2code(newState) 
                newCode = bigRelSpace[tuple(state.tolist())][j]
                #code for new state obtained
                newPos = state2pos(newState) 
                #new motor positions in the new state obtained
                new_x_c = equilibrium_cargo_modified(motor, newPos, Fload, tol) 
